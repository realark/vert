...
(defgeneric world-to-screen-cords (game-object camera)
  (:documentation "Return x-y screen coordinates of the world object.")
  (:method ((object game-object) (camera simple-camera))
    (declare (optimize (speed 3)
                       (space 3))
             (sdl-rectangle-drawable object)
             (simple-camera camera))
    (with-accessors ((drawable-x x) (drawable-y y)) object
      (with-accessors ((scale scale) (camera-x x) (camera-y y)) camera
        ...
        (the (values screen-unit screen-unit)
             (values (ceiling (* scale (- drawable-x camera-x)))
                     (ceiling (* scale (- drawable-y camera-y)))))))))
