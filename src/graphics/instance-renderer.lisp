               ...
               (locally (declare ((integer 0 #.+max-instance-size+) drawable-index))
                 (let ((drawable (elt objects drawable-index))
                   ...
                   (let* ((source (or (sprite-source drawable)
                      ...
                      ;; x y width height. Coerce to single-float BEFORE dividing
                      ;; so integer/integer never produces a (heap-allocated)
                      ;; ratio. This runs for every drawable every frame.
